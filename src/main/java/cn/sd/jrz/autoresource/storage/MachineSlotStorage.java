    ...
    public CompoundTag serializeNBT() {
        ...
        for (int i = 0; i < stacks.size(); i++) {
            if (!stacks.get(i).isEmpty()) {
                ...
                // 通过 ItemStack.CODEC 将 ItemStack 序列化为 NBT，再取内部子标签写入
                ItemStack.CODEC.encodeStart(net.minecraft.nbt.NbtOps.INSTANCE, stacks.get(i))
                        .result()
                        .ifPresent(tag -> {
                            if (tag instanceof CompoundTag stackTag) {
                                for (String key : stackTag.keySet()) {
                                    itemTag.put(key, stackTag.get(key));
                                }
                            }
                        });
    ...
    public MachineSlotStorage deserializeNBT(CompoundTag nbt) {
        ...
        for (int i = 0; i < tagList.size(); i++) {
            ...
            if (slot >= 0 && slot < stacks.size()) {
                stacks.set(slot, ItemStack.CODEC.parse(net.minecraft.nbt.NbtOps.INSTANCE, itemTags)
                        .result()
