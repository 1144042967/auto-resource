...
public class BlockGeneratorItem extends BlockItem {
    ...
    public void appendHoverText(@NotNull ItemStack stack, @NotNull Item.TooltipContext context, @NotNull TooltipDisplay display, @NotNull Consumer<Component> adder, @NotNull TooltipFlag flagIn) {
        ...
        if (tagData != null) {
            tag = tagData.getUnsafe();
            double[] outRef = {output};
            long[] blockRef = {block};
            tag.getLong("output").ifPresent(l -> outRef[0] = l / 1000D);
            tag.getLong("block").ifPresent(l -> blockRef[0] = l / 1000);
            output = outRef[0];
            block = blockRef[0];
        ...
        // 数值行使用机器主题色
        adder.accept(Component.translatable("item.autoresource.block_generator.tooltip.block", block).withStyle(config.getThemeColor()));
        adder.accept(Component.translatable("item.autoresource.block_generator.tooltip.output", output).withStyle(config.getThemeColor()));
        if (output < config.getMax()) {
            adder.accept(Component.translatable("item.autoresource.block_generator.tooltip.growth", percent).withStyle(ChatFormatting.GREEN));
        } else {
            adder.accept(Component.translatable("item.autoresource.block_generator.tooltip.growth_max").withStyle(ChatFormatting.GOLD));
        }
        adder.accept(Component.translatable("item.autoresource.block_generator.tooltip.step", second, step / 1000D).withStyle(ChatFormatting.GRAY));
        // 标记槽内容物描述（兼容未标记/为空的情况）
        if (tag != null && tag.contains("markerSlot")) {
            tag.getCompound("markerSlot").ifPresent(markerTag -> {
                MachineSlotStorage marker = new MachineSlotStorage(1).deserializeNBT(markerTag);
                ItemStack marked = marker.getItem(0);
                if (!marked.isEmpty()) {
                    adder.accept(Component.translatable("item.autoresource.block_generator.tooltip.marked", marked.getHoverName()).withStyle(ChatFormatting.GOLD));
                } else {
                    adder.accept(Component.translatable("item.autoresource.block_generator.tooltip.unmarked").withStyle(ChatFormatting.GRAY));
                }
            });
        } else {
            adder.accept(Component.translatable("item.autoresource.block_generator.tooltip.unmarked").withStyle(ChatFormatting.GRAY));
        ...
        if (!items.isEmpty()) {
            adder.accept(Component.translatable("item.autoresource.block_generator.tooltip.blocks").withStyle(ChatFormatting.GRAY));
            ...
            for (int i = 0; i < visible; i++) {
                ...
                if (++count == PER_ROW) {
                    adder.accept(Component.literal(sb.toString()).withStyle(ChatFormatting.GRAY));
            ...
            if (items.size() > MAX_ITEMS) {
                adder.accept(Component.translatable("item.autoresource.block_generator.tooltip.blocks_more", items.size(), MAX_ITEMS).withStyle(ChatFormatting.GRAY));
            }
        }
        adder.accept(Component.translatable("item.autoresource.block_generator.tooltip.set_block").withStyle(ChatFormatting.DARK_GRAY));
        adder.accept(Component.translatable("item.autoresource.block_generator.tooltip.tip").withStyle(ChatFormatting.DARK_GRAY));
    }
        ...
        for (String entry : entries) {
            ...
            if (id.startsWith("#")) {
                ...
                if (loc != null) {
                    ...
                    registry.get(tagKey).ifPresent(named -> {
                ...
                if (loc != null) {
                    registry.getOptional(loc).ifPresent(item -> {
