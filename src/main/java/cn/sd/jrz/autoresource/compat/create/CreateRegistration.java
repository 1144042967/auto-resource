 ...
 * <p>26.1.2：Block/Item 构造要求 Properties.setId(ResourceKey)；vanilla BlockEntityType.Builder
 * 已被移除，改用 {@link FabricBlockEntityTypeBuilder}；菜单类型改用 {@link ExtendedMenuType} +
 * {@link BlockPos#STREAM_CODEC} 直接传递坐标。
 */
public final class CreateRegistration {

    // 26.1.2：Block 构造要求 Properties.setId(ResourceKey)，构建独立 Properties（共享无法区分 id）
    ...
    public static void register() {
        ...
        // 26.1.2：vanilla BlockEntityType.Builder 移除，改用 FabricBlockEntityTypeBuilder
        ...
        Registration.WATER_WHEEL_MOTOR_MENU = Registry.register(BuiltInRegistries.MENU, Registration.id("water_wheel_motor"),
                new ExtendedMenuType<>((id, inv, pos) -> new WaterWheelMotorMenu(id, inv, pos), BlockPos.STREAM_CODEC));
    }
}
