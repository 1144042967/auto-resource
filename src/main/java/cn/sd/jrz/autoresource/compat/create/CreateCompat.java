...
/**
 * Create 联动入口：判断 Create（机械动力飞越版，mod id 仍为 create）是否加载、懒加载并缓存水车/大水车物品引用。
...
public final class CreateCompat {
     ...
     * <p>当编译期未提供 create-fly jar 时，本类及其依赖的 {@code WaterWheelMotorBlock/Entity/Item/Menu}
    ...
    public static void invokeRegistration(String method, Class<?>[] paramTypes, Object[] args) {
        try {
            ...
            // 编译时未引入 create-fly jar，对应类未被打进本 mod；无 Create 联动，忽略
            AutoResource.LOGGER.info("Create 联动类未打入当前 jar（编译期未提供 create-fly jar），跳过 register");
     ...
     * <p>对应 client 端 compat 类在编译期未提供 create-fly jar 时同样会被排除，反射不到时静默跳过。
     */
    public static void invokeClient(String method, Class<?>[] paramTypes, Object[] args) {
        try {
            ...
            AutoResource.LOGGER.info("Create 联动客户端类未打入当前 jar（编译期未提供 create-fly jar），跳过 " + method);
        } catch (ReflectiveOperationException e) {
    ...
    private static Item lookup(String namespace, String path) {
        // 1.21.11：Registry.get 返回 Optional，未注册时为空 → 归一化为 null
        Item item = BuiltInRegistries.ITEM.getOptional(Identifier.fromNamespaceAndPath(namespace, path)).orElse(null);
