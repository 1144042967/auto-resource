 ...
 * 变换只用标准 PoseStack 方法 + {@code com.mojang.math.Quaternion}（1.19.2 没有 org.joml），
 * 转速经 currentSpeed() 读取（随 SmartBlockEntity 同步包下发，客户端无需额外网络同步）。
 */
@OnlyIn(Dist.CLIENT)
public class WaterWheelMotorRenderer implements BlockEntityRenderer<WaterWheelMotorEntity> {
    ...
    private void renderTextOnFace(Component text, Direction output, Direction face, PoseStack poseStack, MultiBufferSource buffer, int light) {
        ...
        poseStack.mulPose(Vector3f.ZP.rotation(textAngle(output, face)));
        // 缩放字体（Y 取反，保证文字不镜像）
        poseStack.scale(TEXT_SCALE, -TEXT_SCALE, TEXT_SCALE);
        // 居中：水平按文字实际宽度，垂直按 8px 字高中心
        // 1.19.2 的 drawInBatch 第 8 参是 seeThrough 布尔（不是 1.20 的 Font.DisplayMode），NORMAL 对应 false
     ...
     */
    private static Quaternion faceRotation(Direction face) {
        return switch (face) {
            case UP -> Vector3f.XP.rotation(-90f * DEG_TO_RAD);
            case DOWN -> Vector3f.XP.rotation(90f * DEG_TO_RAD);
            default -> Vector3f.YP.rotation(SIDE_ROT_Y[face.ordinal()] * 90f * DEG_TO_RAD);
        };
    }
    ...
    private static float textAngle(Direction output, Direction face) {
        ...
        // 1.19.2 的 Quaternion 没有 conjugate()，conj() 是就地取共轭，必须先 copy 出副本再取
        Quaternion inverse = faceRotation(face).copy();
        inverse.conj();
        f.transform(inverse);
