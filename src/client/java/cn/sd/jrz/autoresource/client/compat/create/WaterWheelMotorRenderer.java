...
public class WaterWheelMotorRenderer implements BlockEntityRenderer<WaterWheelMotorEntity> {
     ...
     *
     * 注意：26.x 的 Font 不再提供 drawInBatch(world) —— 改为 prepareText + TextRenderable.render 的渲染管线。
     * 由于本渲染器只在 Create 加载时参与编译，而世界文字绘制与当前 RenderPipeline 重写工作量较大，
     * 这里暂时只保留姿态变换并跳过文字实际绘制（编译通过即可，运行时转速显示留作 TODO）。
     */
    private void renderTextOnFace(Component text, Direction output, Direction face, PoseStack poseStack, MultiBufferSource buffer, int light) {
        ...
        // TODO(26.3): 用 Font.prepareText + TextRenderable.render 替代已删除的 drawInBatch，
        // 渲染参数需配合当前 RenderPipeline 与 GPU 纹理接口改造。当前为保证编译通过暂时 no-op。
        poseStack.popPose();
    }
